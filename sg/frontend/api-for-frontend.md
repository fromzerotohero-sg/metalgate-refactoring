...
The frontend is served from the apex, `https://fromzerotohero.io`, and the API from
...
what makes the shared login work. (`v2.` is the API's permanent hostname; the frontend
answered on `silver.` during the cutover.)
...
`plan`, `usage` and `upgrade` carry the same objects documented in §6; `plan` and
`upgrade` may be `null`.
...
`amount` is negative for spend and positive for a grant. `type` is one of:

| `type` | Meaning |
|---|---|
| `subscription_grant` | A plan allowance granted for a new billing period |
| `purchase` | Legacy one-off credit packs, from before subscriptions |
| `bonus` | A granted temporary credit (referral, admin, promo) |
| `deduction` | Credits spent by a platform |

Only the 50 most recent entries are returned, and there is no pagination yet — if
the UI needs a full history, that has to be added server-side first.
...
On return, the account page is opened with `?subscription=success` or
...
The return URLs are built **server-side from configuration** (`SG_DASHBOARD_URL`), not
from your `Origin`, so they always point at SilverGate's own account page even when a
platform started the checkout. There is nothing for you to pass in.
...
Honour `redirect` when it is a site-relative path (`/something`) and fall back to the
account page otherwise — a platform that starts the signup passes it so the user lands
back inside that platform. Never follow it if it is an absolute URL: that would turn
the verification link into an open redirect.

Server-side there is an alternative: setting `SG_EMAIL_VERIFY_VIA_API=true` points the
email at `GET /api/auth/verify?token=…` instead, which verifies and creates the session
in one navigation with no page in between. That is an escape hatch for when the
frontend is unavailable; the frontend-page flow above is the one in use.
...
the user to `redirect`, or to the account page.
...
Resends the verification email. **Always returns `200`** with the same message,
whether the address is pending, already verified, or unknown — like
`forgot-password`, this endpoint deliberately does not reveal which emails are
registered. Show the returned `message` verbatim; do not add a "no such account"
state, because the API never says that.
...
**After 5 wrong attempts the code is burned** and cannot be used even if the next
attempt is correct — `400` in every case. On repeated `400`s, prompt the user to
request a new code rather than to check their typing; the API deliberately does not
distinguish a wrong code from an exhausted one.
...
| Transaction history had no type discriminator | `type` is returned (`subscription_grant`, `purchase`, `bonus`, `deduction`) |
