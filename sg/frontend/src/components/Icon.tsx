...
const PATHS: Record<Exclude<IconName, "apple" | "playStore">, ReactElement> = {
...
/**
 * Google's "G", in its brand colours.
 *
 * Deliberately not an `IconName`: every icon in that set is a one-colour glyph
 * driven by `currentColor` so the design system can recolour it, and Google's
 * mark must not be recoloured. A Google button whose logo has been tinted navy
 * is exactly what the brand guidelines forbid.
 */
