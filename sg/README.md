...
| Method | Path | Notes |
|---|---|---|
| GET | `/api/admin/stats` · `/users` · `/users/<id>` · `/transactions` · `/activity` · `/revenue` | Customer analytics. `/stats` also returns `subscriptions` (per-plan counts, `past_due`, `canceling`, `new_30d`, `canceled_30d`), `mrr`, `revenue_30d`, `credits_spent_30d`, `paying_users`, `open_conversations`/`unread_messages`, `logged_today` (last_login since UTC midnight — `active_today` stays rolling 24h), `users_spent_today` (distinct credit spenders since UTC midnight). `/revenue?days=` (1..365) is a zero-filled daily series `[{date, revenue}]`, same shape as `/activity`. `/users` and `/transactions` take `sort`/`order` (whitelisted columns) plus date-range and field filters; `/streamers` takes `sort`/`order` |
