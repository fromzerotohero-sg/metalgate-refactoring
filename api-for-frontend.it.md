...
| | |
|---|---|
| Produzione | `https://api.fromzerotohero.io/api` |
...
significa `GET https://api.fromzerotohero.io/api/session`.
...
`amount` è negativo per una spesa e positivo per un accredito.
...
Al ritorno, la dashboard viene aperta con `?subscription=success` o
...
Best practice: il link nell'email punta **direttamente all'API**,
`GET /api/auth/verify?token=…`, che verifica l'account, **crea la sessione** e
reindirizza il browser nella dashboard. L'utente si registra una volta e non gli viene
mai più chiesto di fare login.

Se preferisci una pagina tua, può leggere `?token=` e chiamare:
...
Reinvia l'email di verifica. `200` sempre per una registrazione in sospeso;
`{"message": "Email already verified"}` se è già stata fatta; `404` se sconosciuto.
